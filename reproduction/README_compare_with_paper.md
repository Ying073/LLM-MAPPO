...
新版已按论文正文、Algorithm 3、表 I/II 与附录 Eq. (34) 修正核心执行链，并在 austlab 完成 3,000-episode pilot 与 28,000-episode formal。formal 显示 outer 150 后出现明显策略塌缩，因此最终 checkpoint 不能视为最佳策略；当前稳定性修订将先通过新的 3,000-episode pilot 验证。
...
| Actor 目标 Eq. (23) | PPO clipped surrogate objective；论文未公开 entropy 系数 | 通用训练器默认 `entropy_coef=0`；远程实验显式设为 `0.01`，作为防止实测策略塌缩的复现假设 |
| 正式模型 | 训练达到 28,000 episodes 后输出 optimal Actor/Critic；未描述中途验证选模 | 保存周期快照，用独立验证 seeds 100–107 选最佳冻结策略，再用隔离测试 seeds 评估；这是新增复现协议 |
...
1. austlab 的 28,000-episode formal 使用 `entropy_coef=0`，同一验证集上从 outer 150 的 98.33% 成功率退化到 outer 1400 的 90.83%，初始状态动作熵从 0.658 降至 0.115。新实验显式使用 `entropy_coef=0.01` 并执行独立验证选模；这是根据复现实证增加的稳定性设置，不是论文公开参数。
...
- Formal：28,000 episodes；训练完成后用 seeds 100–107 选择冻结策略，再在此前未用于诊断的 seeds 400–407 上测试并报告 mean±std。
...
训练结束后使用验证 seeds 100–107 对阶段策略选模，优先级依次为：全部无碰撞、找到的不同目标数、终局区域不确定度、搜索时间、累计搜索指示。pilot 使用 seeds 300–307 测试，formal 使用 seeds 400–407 最终测试；原 seeds 0–7 和 200–207 已参与历史诊断，不再视为未见测试集。该 checkpoint 选择协议是本复现新增的防过拟合措施，论文没有公开对应细节。
