...
论文对齐修订与 AutoDL GPU smoke 已完成，当前等待 3,000-episode pilot。旧版 150-episode 与 m12 数据只作历史记录，不能用于论文结论。
...
4. 运行 `bash cloud/cloud_run.sh`，完成 3,000-episode pilot。
...
论文没有给出 Eq. (34) 中 `chi_th`、高空条件概率和安全距离的全部数值；当前实现采用显式默认假设。论文的网格尺寸、1 s 时间步与 10 m/s UAV 速度同“每步移动到相邻 100 m 网格”的描述存在尺度张力，实现保留离散相邻动作并按论文速度计算能耗。DPES 使用融合后的全局目标概率构造信息素，再向各 UAV 提供局部 DP patch；这是根据算法流程作出的实现解释。
